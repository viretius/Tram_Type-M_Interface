...
void handshake() 
{
  ...
  { 
    ...
    port = mcp_list[i].mcp.read();

    for (t = 0; t < 16; t++) 
    {
      if (strncmp(mcp_list[i].address[t], "-1", 2) == 0) continue;
      if (!CHECK_BIT(mcp_list[i].portMode, t)) continue;   //pin set as output
      if (VERBOSE) USBSerial.println("found input pin");

      button_state = CHECK_BIT(port, t);
      if (VERBOSE) USBSerial.println("writing bit...");
      bitWrite(mcp_list[i].last_reading, t, button_state);
      ...
      memset(&data[0], '\0', 5);
      button_state ? strcpy(data, "0000") : strcpy(data, "0001") ;   //check wether the pin changed from 0 to 1 or 1 to 0 and store according char to "data" variable, inverted because of the implementation in the simulation programm
