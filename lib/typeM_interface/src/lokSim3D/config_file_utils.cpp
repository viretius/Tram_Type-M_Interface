...
bool load_config()
{
  ...
  for (t = 0; t < mcp_parse.getRowsCount(); t++) 
  {
    ...
    {      
      ...
      else 
      {
        ...
        mcp_list[i2c[t] - MCP_I2C_BASE_ADDRESS].address[pin[t]] = new char[len]; //address konsists of 2 digits and null-terminator 
  ...
  for (t = 0; t < pcf_parse.getRowsCount(); t++)
  {
    ...
    }
    {
      ...
      { //
        size_t len = strlen(address[t]+1);
        pcf_list[i2c[t] - PCF_I2C_BASE_ADDRESS].address[pin[t]] = new char[len]; //address konsists of 2 digits and null-terminator 
